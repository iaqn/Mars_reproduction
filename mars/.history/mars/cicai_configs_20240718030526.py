...
MAX_NUM_ITERATIONS = 300000 #600000
STEPS_PER_SAVE = 2000
            ...
            datamanager=MarsDataManagerConfig(
                dataparser=MarsVKittiDataParserConfig(
                    ...
                    car_object_latents_path=Path(
                        "latent_codes/vkitti/car-object-latents/latent_codes02.pt"
                    ),
                    split_setting="reconstruction",
                    car_nerf_state_dict_path=Path("latent_codes/vkitti/car-nerf-state-dict/epoch_805.ckpt"),
                ),
        ...
        pipeline=MarsPipelineConfig(
            datamanager=MarsDataManagerConfig(
                dataparser=MarsVKittiDataParserConfig(
                    ...
                    car_object_latents_path=Path(
                        "latent_codes/vkitti/car-object-latents/latent_codes02.pt"#"/DATA_EDS/liuty/ckpts/pretrain/car_nerf/vkitti/latents/latent_codes02.pt"
                    ),
                    split_setting="reconstruction",
                    car_nerf_state_dict_path=Path("latent_codes/vkitti/car-nerf-state-dict/epoch_805.ckpt"),#/DATA_EDS/liuty/ckpts/pretrain/car_nerf/vkitti/epoch_805.ckpt
                    semantic_path=Path("datasets/vkitti/Scene02/clone/frames/classSegmentation"),#/data22/DISCOVER_summer2023/xiaohm2306/Scene02/clone/frames/classSegmentation
                ),
                train_num_rays_per_batch=4096,
                eval_num_rays_per_batch=4096,
                camera_optimizer=CameraOptimizerConfig(mode="off"),
...
'''
###################-------------------------VIKITTI ID 1--------------------------##########################
#open the sky model and set use_sky_model=True
VKITTI_Recon_Mars_Car_Depth_Semantic = MethodSpecification(
    config=TrainerConfig(
        method_name="mars-vkitti-car-depth-recon-semantic",
        steps_per_eval_image=STEPS_PER_EVAL_IMAGE,
...
KITTI_Recon_Mars_Car_Depth = MethodSpecification(
    config=TrainerConfig(
        ...
        pipeline=MarsPipelineConfig(
            datamanager=MarsDataManagerConfig(
                dataparser=MarsKittiDataParserConfig(
                    ...
                    use_depth=True,
                    car_object_latents_path=Path(
                        "latent_codes/KITTI-MOT/car-object-latents/latent_codes06.pt"
                    ),
                    split_setting="reconstruction",
                    car_nerf_state_dict_path=Path("latent_codes/KITTI-MOT/car-nerf-state-dict/epoch_670.ckpt"),
                    scale_factor=0.1,
                ),
                train_num_rays_per_batch=4096,
                eval_num_rays_per_batch=4096,
        ...
        optimizers={
            "background_model": {
                "optimizer": RAdamOptimizerConfig(lr=1e-3, eps=1e-15),
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-5, max_steps=200000),#200000
            },
            "learnable_global": {
                "optimizer": RAdamOptimizerConfig(lr=1e-3, eps=1e-15),
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-5, max_steps=200000),#200000
...
KITTI_Recon_Mars_Car_Depth_Semantic = MethodSpecification(
    config=TrainerConfig(
        ...
        pipeline=MarsPipelineConfig(
            datamanager=MarsDataManagerConfig(
                dataparser=MarsKittiDataParserConfig(
                    use_car_latents=True,
                    use_depth=True,
                    use_semantic=True, #new add
                    semantic_mask_classes=["van","Undefined"],#new add
                    car_object_latents_path=Path(
                        "latent_codes/KITTI-MOT/car-object-latents/latent_codes06.pt"
                    ),
                    split_setting="reconstruction",
                    car_nerf_state_dict_path=Path("latent_codes/KITTI-MOT/car-nerf-state-dict/epoch_670.ckpt"),
                    semantic_path=Path("datasets/KITTI_MOT/panoptic_maps"), #new add and delete scale_factor=0.1
                ),
                train_num_rays_per_batch=4096,
                eval_num_rays_per_batch=4096,
                camera_optimizer=CameraOptimizerConfig(mode="off"),
            ),
            model=SceneGraphModelConfig( #more modify
                background_model=SemanticNerfWModelConfig(
                    num_proposal_iterations=1,
                    num_proposal_samples_per_ray=[48],
                    num_nerf_samples_per_ray=97,
        ...
        optimizers={
            "background_model": {
                "optimizer": RAdamOptimizerConfig(lr=1e-3, eps=1e-15),
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-5, max_steps=100000),#200000
            },
            "learnable_global": {
                "optimizer": RAdamOptimizerConfig(lr=1e-3, eps=1e-15),
                "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-5, max_steps=100000),#200000
